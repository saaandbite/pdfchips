...
export default function PptToPdf() {
  ...
  const handlePick = async () => {
    const selected = await open({
      multiple: false,
      filters: [{ name: 'Presentation Files', extensions: ['pptx', 'ppt', 'odp'] }],
    });
    if (!selected) return;
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const outputPath = `${tempDir}/${file.name.replace(/\.(pptx|ppt|odp)$/i, '')}.pdf`;
      setProgress(60);
      const res = await invoke<ConvertResult>('ppt_to_pdf', {
        inputPath: file.path,
        outputPath,
      });
      setProgress(100);
  ...
  const handleSave = async () => {
    if (!result) return;
    const baseName = file?.name.replace(/\.(pptx|ppt|odp)$/i, '') || 'output';
  ...
  return (
    <>
      <div className="page-header">
        ...
        <h2>📊 PPT → PDF</h2>
        <p>Konversi presentasi PowerPoint (.pptx, .ppt, .odp) menjadi PDF.</p>
      </div>
        ...
        <div className="tool-page">
          {!file && (
            <div className="dropzone" onClick={handlePick}>
              ...
              <h3>Pilih File Presentasi</h3>
              <p>Klik untuk memilih file .pptx / .ppt / .odp dari komputer Anda</p>
            </div>
          )}
          {file && (
          ...
          {processing && <ProgressBar progress={progress} status="Mengkonversi presentasi ke PDF dengan LibreOffice..." />}
          ...
          <div className="options-panel" style={{ marginTop: '20px' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              Menggunakan <strong>LibreOffice Impress</strong> untuk konversi. Mendukung .pptx, .ppt, .odp.<br />
