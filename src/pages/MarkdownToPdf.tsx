...
export default function MarkdownToPdf() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const outputPath = `${tempDir}/${file.name.replace(/\.(md|markdown)$/i, '')}.pdf`;
      setProgress(60);
      const res = await invoke<ConvertResult>('markdown_to_pdf', {
        inputPath: file.path,
        outputPath,
      });
      setProgress(100);
  ...
  const handleSave = async () => {
    if (!result) return;
    const baseName = file?.name.replace(/\.(md|markdown)$/i, '') || 'output';
  ...
  return (
    <>
      <div className="page-header">
        ...
        <h2>Ⓜ Markdown → PDF</h2>
        <p>Konversi file Markdown (.md) menjadi dokumen PDF yang rapi.</p>
      </div>
        ...
        <div className="tool-page">
          {!file && (
            <div className="dropzone" onClick={handlePick}>
              ...
              <h3>Pilih File .md</h3>
              <p>Klik untuk memilih file Markdown dari komputer Anda</p>
            </div>
          )}
          {file && (
          ...
          {processing && <ProgressBar progress={progress} status="Mengkonversi Markdown ke PDF..." />}
          ...
          <div className="options-panel" style={{ marginTop: '20px' }}>
            ...
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              Menggunakan <strong>pandoc</strong> (diutamakan) atau <strong>LibreOffice</strong> sebagai fallback.<br />
              Install pandoc untuk hasil terbaik:<br />
